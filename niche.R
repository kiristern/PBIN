...
sparc <- print(sparc[sparc$pvalue_adjusted <= .Options$sig_pval, ], row.names=FALSE)
...
# dim(gendist)
dim(vir.corr.tab)
names(vir.corr.tab)[names(vir.corr.tab) == "weight"] <- "covariance"
# dim(gendist.table)
...
fit <- lm(covariance ~ gen.dist , data = gendist.covar)
(coefs <- coef(fit))
#get r2
(r2 = signif(summary(fit)$adj.r.squared))
#get p-value
(pval <- signif(summary(fit)$coefficients[2,4]))
ggplot(gendist.covar, aes(x=gen.dist, y=covariance, group=gen.dist))+
  geom_boxplot()+
  geom_jitter()+
...
#### filter limits ####
gc.filt <- gendist.covar %>%
  filter(gen.dist > 0 & gen.dist <0.05)
dim(gc.filt)

fit <- lm(covariance ~ gen.dist , data = gc.filt)
coefs <- coef(fit)
summary(fit)
#get r2
(r2 = signif(summary(fit)$adj.r.squared))
#get p-value
(pval <- signif(summary(fit)$coefficients[2,4]))
ggplot(gc.filt, aes(x=gen.dist, y=covariance, group=gen.dist))+
  geom_boxplot()+
  geom_jitter()+
  geom_smooth(method = "lm", se=F, color="red", aes(group=1))+
  labs(title = paste("Adj R2 = ", r2,
...
fit <- lm(correlation ~ gen.dist , data = gendist.sparcc)
(coefs <- coef(fit))
#get r2
(r2 = signif(summary(fit)$adj.r.squared))
#get p-value
(pval <- signif(summary(fit)$coefficients[2,4]))
ggplot(gendist.sparcc, aes(x=gen.dist, y=correlation, group=gen.dist))+
  geom_point()+
  geom_boxplot()+
  geom_smooth(method = "lm", se=F, color="red", aes(group=1))+
  labs(title = paste("Adj R2 = ", r2,
...
gs.filt.pos <- gendist.sparcc %>%
  filter(correlation > 0 & gen.dist <0.05)

fit <- lm(correlation ~ gen.dist , data = gs.filt.pos)
(coefs <- coef(fit))
#get r2
(r2 = signif(summary(fit)$adj.r.squared))
#get p-value
(pval <- signif(summary(fit)$coefficients[2,4]))
ggplot(gs.filt.pos, aes(x=gen.dist, y=correlation, group=gen.dist))+
  geom_point()+
  geom_boxplot()+
  geom_jitter()+
  geom_smooth(method = "lm", se=F, color="red", aes(group=1))+
...
gs.filt.neg <- gendist.sparcc %>%
  filter(correlation < 0 )

fit <- lm(correlation ~ gen.dist , data = gs.filt.neg)
(coefs <- coef(fit))
#get r2
(r2 = signif(summary(fit)$adj.r.squared))
#get p-value
(pval <- signif(summary(fit)$coefficients[2,4]))
ggplot(gs.filt.neg, aes(x=gen.dist, y=correlation, group=gen.dist))+
  geom_point()+
  geom_boxplot()+
  geom_jitter()+
  geom_smooth(method = "lm", se=F, color="red", aes(group=1))+
