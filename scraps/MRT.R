...
spid <- data.matrix(spider[,1:12])
spid_env <- data.matrix(spider[,13:18])

fit_spider <- mvpart(spid ~ spid_env, spider, legend=T, margin=0.01, cp=0, xv="pick",
       xval=nrow(sp.asv.rm), xvmult=100, which=4, big.pts=T, bars=F, pca=T)
#rpart.pca(fit_spider, interact = T, wgt.ave = T) #takes toooooo long!
summary(fit_spider)
