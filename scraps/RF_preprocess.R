...
# write.table(sumdoli_keep, "sumdoli.txt")
# write.table(viral4doli_keep, "viral4doli.txt")
write.csv(sumdoli_keep, "sumdoli.csv")
write.csv(viral4doli_keep, "viral4doli.csv")
...
### micro RF  ####
(micro_rf <- randomForest(summicro_keep ~ ., #remove $sum if didn't as.df
...
### doli RF  ####
(doli_rf <- randomForest(sumdoli_keep ~ ., #remove $sum if didn't as.df
