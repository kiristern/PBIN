...
#filter viral count by pelagic (ie. select cols that match rownames from litt. or pel.)
...
count_phyP <- otu_table(vir_count_pelagic, taxa_are_rows=T)
...
viral_physeqP <- phyloseq(count_phyP, sample_info, virTree, mock_taxa)

#check data
print(viral_physeqP)
#view ASV count (top 10)
sort(taxa_sums(viral_physeqP), decreasing = T)[1:10]
...
library(otuSummary)
condrare_viralP <- rareBiosphere(vir_count_pelagic)

((nrow(condrare_viralP$CRT))+
    (nrow(condrare_viralP$PERare))+
    (nrow(condrare_viralP$otherRare)))/nrow(vir_count_pelagic)
...
######## NON RARE ANALYSIS ###########
library(microbiome)
virpsP_helli <- transform(viral_physeqP, transform = "hellinger", target = "OTU")
otutabP <- virpsP_helli %>% otu_table()

library(vegan)
#the sum of an OTU across all samples is greater than 0.005% of all OTUs 
minTotRelAb = 5e-5
P = taxa_sums(otutabP)
keepTaxaP = taxa_names(otutabP)[which((P/sum(P)) > minTotRelAb)] 
nonrareP = prune_taxa(keepTaxaP, otutabP)
...
# filt_virseqP <- filter_taxa(viral_physeqP, function(x) sum(x > 1) > (0.10*length(x)), TRUE)
...
top20.p <- read.csv("top20_virP_blastn_nov26.csv")
head(top20.p, n=2)
...
asv_tax.l <- vir_pel[,(colnames(vir_pel) %in% top20.l$query)]
nrow(asv_tax.l)
ncol(asv_tax.l)
...
ggplot(df.l, aes(x = samples.l, y = abundance, fill = ASV_ID.l))+
  ...
  ggtitle("Relative abundance of pelagic viral ASV by date")+
...
#simple RDA
simpleRDA <- rda(sp.asv.rm ~., env_vars)
summary(simpleRDA)
screeplot(simpleRDA)
