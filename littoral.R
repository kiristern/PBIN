...
viral_physeqL <- phyloseq(count_phyL, sample_info, virTree, mock_taxa)

#check data
print(viral_physeqL)
#view ASV count (top 10)
sort(taxa_sums(viral_physeqL), decreasing = T)[1:10]

abundL <- viral_physeqL %>% otu_table()
dim(abundL)
...
library(otuSummary)
condrare_viralL <- rareBiosphere(vir_count_littoral)
...
((nrow(condrare_viralL$CRT))+
    ...
    (nrow(condrare_viralL$otherRare)))/nrow(vir_count_littoral)
...
library(microbiome)
virpsL_helli <- transform(viral_physeqL, transform = "hellinger", target = "OTU")
helliL <- virpsL_helli %>% otu_table()
dim(helliL)
...
minTotRelAb = 5e-3
L = taxa_sums(helliL)
keepTaxaL = taxa_names(helliL)[which((L/sum(L)) > minTotRelAb)] 
#keepTaxaL = (xL/sum(xL)) > minTotRelAb #different way to compute line above
nonrare_helliL = prune_taxa(keepTaxaL, helliL)
dim(nonrare_helliL)
...
# filt_virseqL <- filter_taxa(viral_physeqL, function(x) sum(x > 1) > (0.10*length(x)), TRUE)


# rm asv not seen more than at least 10% in all samples
abund_paL <- decostand(abundL,"pa") #pa: scale x to presence/absence scale (1/0) #ensure against taking ASV with large amounts in just a few samples
abund_pa_10L <- abundL[(rowSums(abund_paL)/ncol(abund_paL) >= 0.10),] #select only asv present in more than 10% of samples
nonrare_helliL2 <- helliL[rownames(abund_pa_10L),] #select asvs present in more than 10% of samples from helli transformed
dim(nonrare_helliL2)
...
###### RegTree HELLINGER NON-RARE ######




#### ALPHA DIV & BETA DIV (no transformations or filtering) ####
library(breakaway)
...
#### ML (hellinger transformed and rm rare) ####
#RF
