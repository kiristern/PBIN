...
head(ASV_count)
meta <- read.csv("metadata_w_cmd.csv", row.names = 1, header = T)
head(meta)
...
row.names(mock_taxa) <- str_remove(rownames(mock_taxa), "s__")
...
# #Littoral
# count_phy_L <- otu_table(vir_count_littoral, taxa_are_rows=T)
# sample_info_L <- sample_data(meta_littoral)
# virTree <- read_tree("viral_tree")
# #add to phyloseq object
# viral_physeq_L <- phyloseq(count_phy_L, sample_info_L, virTree)
# #check data
# print(viral_physeq_L)
# #view ASV count (top 10)
# sort(taxa_sums(viral_physeq_L), decreasing = T)[1:10]
# 
# 
# #repeat for pelagic
# #add ASV count table, metadata, virTree to phyloseq table
# count_phy_P <- otu_table(vir_count_pelagic, taxa_are_rows=T)
# sample_info_P <- sample_data(meta_pelagic)
# virTree <- read_tree("viral_tree")
# #add to phyloseq object
# viral_physeq_P <- phyloseq(count_phy_P, sample_info_P, virTree)
# #check data
# print(viral_physeq_P)
# #view ASV count (top 10)
# sort(taxa_sums(viral_physeq_P), decreasing = T)[1:10]
...
# 
# (vir_cond_rare <- SimpleRareToPrev.f(otu_fp="ASVs_counts_copy.tsv",abund_thresh=0.005, abund_thresh_ALL=TRUE,b_thresh=0.50, rdp_lastcol=FALSE))
#   #[1] 466   6
#   #[1] 184   6
#   #[1] "No. conditionally rare OTUs"
#   #[1] 184
#   #[1] "No. total OTUs"
#   #[1] 5109
#   #[1] "Proportion conditional rare / total OTUs"
#   #[1] 0.03601488
#   #[1] "No singleton OTUs"
#   #[1] 5109
#   #[1] "Proportion conditionally rare / non-singletonOTUs"
#   #[1] 0.03601488
#  #write.csv(vir_cond_rare, "conditionally_rare_viral.csv", row.names = T, quote = F, sep = "\t")
# 
# #select all the conditionally rare ASVs from phyloseq object
# virps <- viral_physeq %>% otu_table()
# crare_virps <- virps[row.names(virps) %in% vir_cond_rare$OTUID, ]
# t_crare_virps <- t(crare_virps) #transpose df
# head(t_crare_virps)
# 
# #store data in timeseries object
# #TODO TIMESERIES ANALYSIS
# 
# (vir_cond_rare_L <- SimpleRareToPrev.f(otu_fp="ASV_count_littoral.txt",abund_thresh=0.005, abund_thresh_ALL=FALSE,b_thresh=0.90, rdp_lastcol=FALSE))
# #write.csv(vir_cond_rare_L, "conditionally_rare_viral_L.csv", row.names = T, quote = F)
# 
# (vir_cond_rare_P <- SimpleRareToPrev.f(otu_fp="ASV_count_pelagic.txt",abund_thresh=0.005, abund_thresh_ALL=FALSE,b_thresh=0.90, rdp_lastcol=FALSE))
# #write.csv(vir_cond_rare_P, "conditionally_rare_viral_P.csv", row.names = T, quote = F)
# 
# 
# 
# #Repeat for bacteria
# (bact_cond_rare <- SimpleRareToPrev.f(otu_fp="cyano/Champ_ASVs_counts_only_noMockorTaxa.txt",abund_thresh=0.005, abund_thresh_ALL=FALSE,b_thresh=0.90, rdp_lastcol=FALSE))
#   # 481   6
#   # 130   6
...
#   # 0.01916274
# #write.csv(bact_cond_rare, "conditionally_rare_bacteria.csv")
# 
# #select all the conditionally rare ASVs from phyloseq object
# baps <- bact_physeq %>% otu_table()
# crare_baps <- baps[row.names(baps) %in% bact_cond_rare$OTUID, ]
# t_crare_baps <- t(crare_baps) #transpose df
# head(t_crare_baps)
# 
# 
# (bac_cond_rare_L <- SimpleRareToPrev.f(otu_fp="bact_count_littoral.txt",abund_thresh=0.005, abund_thresh_ALL=FALSE,b_thresh=0.90, rdp_lastcol=FALSE))
# #write.csv(bac_cond_rare_L, "conditionally_rare_bacteria_L.csv")
# 
# 
# (bac_cond_rare_P <- SimpleRareToPrev.f(otu_fp="bact_count_pelagic.txt",abund_thresh=0.005, abund_thresh_ALL=FALSE,b_thresh=0.90, rdp_lastcol=FALSE))
# 
...
###### Conditionally rare using another package ######
library(otuSummary)
condrare_viral <- rareBiosphere(asv_count_meta)
...
(374+3808+1146)/nrow(asv_count_meta)

# crar_vir_L <- rareBiosphere(vir_count_littoral)
# (297+4138+892)/nrow(vir_count_littoral)
# 
# crar_vir_P <- rareBiosphere(vir_count_pelagic)
# (238+4353+735)/nrow(vir_count_pelagic)
...
#select of OTU data only:
# filt_vir_df <- as.data.frame(filt_virseq %>% otu_table())
...
#   return(m)
# }
# test <- find.top.asv(filt_virseq, 5) # Top 5 Pseudomonas ASVs per sample
# 
...
# divnet_virsp <- divnet(tax_glom(filt_virseq, taxrank = "species"),
#                        base = "ASV_1",
#                        ncores = 4)
# divnet_virsp

# divnet_virsp %>% names
# divnet_virsp$shannon %>% head
# 
# filt_virseq %>% sample_data() %>% head

#check all variables in phyloseq object
sample_variables(filt_virseq)

div_ASV1_years <- filt_virseq %>%
...
estimates2 <- div_ASV1_years$simpson %>% summary %>% select("estimate")
ses2 <- sqrt(div_ASV1_years$`simpson-variance`)
X2 <- breakaway::make_design_matrix(filt_virseq, "Years")
(ba_simpson <- betta(estimates2, ses2, X2)$table)




# #ASV2
# divnet_virsp_asv2_years <- divnet(tax_glom(filt_virseq, taxrank = "species"),
#                                   base = "ASV_2",
#                                   X = "Years",
#                                   ncores = 4)
# divnet_virsp_asv2_years
...
# divnet_virsp_asv2_bloom <- divnet(tax_glom(filt_vir_omitna_bloom, taxrank = "species"),
#                             base = "ASV_2",
#                             X = "bloom2",
#                             ncores = 4)
# divnet_virsp_asv2_bloom
...
#https://microsud.github.io/microbiometutorials.gihub.io/microbiome_tutorial.html
library(microbiome)
summarize_phyloseq(filt_virseq)
...
vir_abund_helli <- filt_virseq %>% otu_table %>% decostand(method = "hellinger")
length(meta)
colnames(meta)
head(env <-  meta[,2:15])

#standardize environmental data
env[,c(6:14)]<-decostand(env[,c(6:14)], method="standardize")
...
dbRDA <- capscale(sp.asv.rm ~ ., data = env_keep, distance="bray")
plot(dbRDA)

# plot the RDA using ggplot (ggord package)
library(ggord)
ggord(dbRDA, env_keep$Years) + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) # looking at the raw code, this is plotting the 'wa scores', the blue dots are different species  
...
#1) add a constant:
dbRDA_constant <- capscale(sp.asv.rm ~., env_keep, distance = "bray", add=TRUE)
plot(dbRDA_constant)
anova(dbRDA_constant)

#2) take the sqrt of dissimilarities
dbRDA_sqrt <- capscale(sp.asv.rm ~., env_keep, dist="bray", sqrt.dist=T)
plot(dbRDA_sqrt)
anova(dbRDA_sqrt)

#3) Do a square root transformation, Wisconsin double standardization (this emphasizes the environmental variables):
dbRDA_metaMDS=capscale(sp.asv.rm ~ ., env_keep, dist="bray", metaMDS=TRUE, sqrt.dist=TRUE) 
plot(dbRDA_metaMDS)
anova(dbRDA_metaMDS)
...
?ordiR2step
ordiR2step(rda(sp.asv.rm~1, data=env_keep), scope=formula(dbRDA), direction= "forward", R2scope=TRUE, pstep=1000)

#the proportion of variation explained by the constraining variables being 0.3542

#retain significant variables only
signif_env <- subset(env_keep, select = c("Years", "Months", "Dissolved_N", "Period", "Dissolved_P"))
...
#with ellipses
ggord(dbrda_env_signif, signif_env$Years) + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) # looking at the raw code, this is plotting the 'wa scores', the blue dots are different species  
...
#simple RDA
simpleRDA <- rda(sp.asv.rm ~., env_keep)
summary(simpleRDA)
screeplot(simpleRDA)
...
# plot the RDA using ggplot (ggord package)
library(ggord)
ggord(simpleRDA, env_keep$Years) + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) # looking at the raw code, this is plotting the 'wa scores', the blue dots are different species  
...
# scaling 2
plot(simpleRDA, display=c("sp", "lc", "cn"), main="Triplot RDA matrix ~ env -scaling2-lc scores", col=env_keep$Years)
arrows(0,0,spe2.sc[,1],spe2.sc[,2], length=0, lty=1,col='red')
...
spe.rda.all <- rda(sp.asv.rm ~ Years + Site + Period + bloom2 + Total_Phosphorus_ug + Total_Nitrogen_mg + Dissolved_P + Dissolved_N + 
                     Cumulative_precipitation_t1_t7_mm + Mean_temperature_t0_t7 + Micro.Abundance, data=env_keep)

# Forward selection using ordistep
ordistep(rda(sp.asv.rm ~., data=env_keep), direction="forward", pstep=1000, R2scop=TRUE) #R2scope only accepts models with lower adjusted R2

# db-RDA
dbRDA2 <- capscale(sp.asv.rm ~ ., data=env_keep, distance = "bray")
dbRDA2
plot(dbRDA2)
summary(dbRDA2)
screeplot(dbRDA2)
...
x <- ordinate(filt_virseq, method = "RDA", distance = "unifrac")
plot_ordination(x)
...
plottop20 <- function(RFm){
  ...
  imp.top20 %>%
    ...
    ggplot(
      aes(x=predictors, y = MeanDecreaseGini)) +
...
# asvnames <- imp.top20$predictors
# r <- rownames(tax_table(RF_ps)) %in% asvnames
# kable(tax_table(RF_ps)[r, ])
...
respM7 <- micro_response[,3]
impVir <- pred_var[c("ASV_2", "ASV_1273"),]
impVir <- t(impVir)

timeseriedf <- merge(respM7, impVir, by="row.names")
